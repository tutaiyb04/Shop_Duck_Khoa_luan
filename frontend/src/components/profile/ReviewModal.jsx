...
const ReviewModal = ({ sellerId, productId, onSuccess }) => {
  ...
  // 👉 1. Đặt mặc định ban đầu là 0 sao (chưa chọn)
  const [rating, setRating] = useState(0);
  const [comment, setComment] = useState('');
  const [loading, setLoading] = useState(false);

  // Hàm xử lý dọn dẹp sạch sẽ khi đóng Modal
  const handleClose = () => {
    setIsOpen(false);
    setComment(''); 
    // 👉 2. Khi đóng, reset số sao về lại 0
    setRating(0); 
  };

  const handleSubmit = async () => {
    // 👉 3. Validate bắt buộc: Phải chọn ít nhất 1 sao mới cho gửi
    if (rating === 0) {
